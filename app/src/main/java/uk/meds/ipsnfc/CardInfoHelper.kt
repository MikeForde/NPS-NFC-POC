...
object CardInfoHelper {
    ...
    fun buildReport(transport: ApduTransport, debug: Boolean = true): Report {
        return try {
            ...
            sb.appendLine("Source: ${transport.name}")
            sb.appendLine()
    ...
    fun buildReport(tag: Tag, debug: Boolean = true): Report {
        val transport = AndroidIsoDepTransport.connect(tag) ?: return Report("IsoDep not supported.")
        return buildReport(transport, debug)
    }
    ...
    private fun detectMimeForKnownLayouts(aid: ByteArray, fileNo: Byte, fileBytes: ByteArray): String? {
        ...
        return when {
            aid.contentEquals(NDEF_APP_AID) && (fileNo == FILE_NDEF || fileNo == FILE_EXTRA) -> parseType4NdefMimeType(fileBytes)
            aid.contentEquals(IPS_AID) && fileNo == 0x01.toByte() -> parseRawNdefRecordMimeType(fileBytes)
            else -> null
        }
    }
    ...
    private fun parseRawNdefRecordMimeType(ndefBytes: ByteArray): String? {
        ...
        val payloadLen = if (sr) ndefBytes[idx++].toInt() and 0xFF else {
            if (idx + 4 > ndefBytes.size) return null
            ((ndefBytes[idx].toInt() and 0xFF) shl 24) or ((ndefBytes[idx + 1].toInt() and 0xFF) shl 16) or ((ndefBytes[idx + 2].toInt() and 0xFF) shl 8) or (ndefBytes[idx + 3].toInt() and 0xFF).also { idx += 4 }
        }
        ...
        idx += typeLen
        if (idLen > 0) { if (idx + idLen > ndefBytes.size) return null; idx += idLen }
    ...
    private fun hexDump(bytes: ByteArray, maxBytes: Int = bytes.size): String {
        ...
        while (offset < n) {
            ...
            for (i in 0 until 16) {
                if (i < lineLen) sb.append(String.format("%02X ", bytes[offset + i])) else sb.append("   ")
            }
            sb.append(" ")
